...
export PORT_SEND_REQUEST=1234
noise-tool activate

mkdir /current-test-results
mkdir -p /test-results

npm run test:functional:update
...
for i in $(seq 1 $EXECUTIONS); do
   ...
   PLAYWRIGHT_JUNIT_OUTPUT_NAME=/current-test-results/results.xml npm run test:functional -- --reporter=junit
   ls
   echo "----"
